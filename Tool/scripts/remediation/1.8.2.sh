#!/usr/bin/env bash
{
    ...
    # Loop over the package list and check if each package is installed
    for l_pn in $l_pcl; do
        $l_pq "$l_pn" > /dev/null 2>&1 && l_pkgoutput="$l_pkgoutput\n - Package: \"$l_pn\" exists on the system\n - checking configuration"
    done

    if [ -n "$l_pkgoutput" ]; then
        l_gdmprofile="gdm"  # Set this to the desired profile name
        l_bmessage="'Authorized uses only. All activity may be monitored and reported'"  # Set to the desired banner message
        ...
        # Check if the keyfile for machine-wide settings exists
        if ! grep -Piq '^\h*banner-message-enable\h*=\h*true\b' /etc/dconf/db/$l_gdmprofile.d/*; then
            echo "creating gdm keyfile for machine-wide settings"

            # Check if any keyfile for machine-wide settings exists
        ...
        # Check if the banner message text
