#!/usr/bin/env bash
{
    # Check if GNOME Desktop Manager is installed. If the package isn't installed, recommendation is Not Applicable.
    ...
    if [ -n "$l_pkgoutput" ]; then
        # Look for idle-delay to determine the profile in use, needed for the remaining tests
        ...
        # Look for lock-delay to determine the profile in use, needed for the remaining tests
        ...
        if [ -d "$l_kfd" ]; then
            ...
            if grep -Prilq '^\h*\/org\/gnome\/desktop\/session\/idle-delay\b' "$l_kfd"; then
                echo " - \"idle-delay\" is locked in \"$(grep -Pril '^\/org\/gnome\/desktop\/session\/idle-delay\b' "$l_kfd")\""
            else
            ...
            echo -e " - \"idle-delay\" is not set, so it cannot be locked\n - Please follow the recommendation 'Ensure GDM screen locks when the user is idle' and follow this recommendation again"
        fi
        
        if [ -d "$l_kfd2" ]; then
            # If the key file directory doesn't exist, options can't be locked
            if grep -Prilq '^\h*\/org\/gnome\/desktop\/screensaver\/lock-delay\b' "$l_kfd2"; then
                echo " - \"lock-delay\" is locked in \"$(grep -Pril '^\/org\/gnome\/desktop\/screensaver\/lock-delay\b' "$l_kfd2")\""
            else
